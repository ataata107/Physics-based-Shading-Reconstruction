...
def grad_loss(pdt, gt, device, direction = "x"):
  if(direction=="x"):
    filter_1 = torch.from_numpy(np.array([[-1,0,1],[-2,0,2],[-1,0,1]])).to(device).view(1, gt.size(1), 3, 3)
  else:
    filter_1 = torch.from_numpy(np.array([[-1,-2,-1],[0,0,0],[1,2,1]])).to(device).view(1, gt.size(1), 3, 3)
